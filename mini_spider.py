...
def main(config_file):
    ...
    config_items = ConfigLoad()
    try:
        config_items.config_file_load(config_file)
    except SpiderException as e:
        logger.error(e.msg)
        return

    # Get config informations
    url_list_file = config_items.url_list_file
    output_directory = config_items.output_directory
    max_depth = config_items.max_depth
    crawl_interval = config_items.crawl_interval
    crawl_timeout = config_items.crawl_timeout
    target_url = config_items.target_url
    thread_count = config_items.thread_count
    ...
    logger.info("Reading seed file %s ..." % url_list_file)
    seed_file = SeedFile()
    try:
        seed_file.load(url_list_file)
    except SpiderException as e:
        logger.error(e.msg)
        return
